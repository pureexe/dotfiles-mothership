...
def showGPUs_fn(cluster, show_json=False, timeout=120):
    ...
    try:
        ...
        proc = subprocess.run(
            ...
            text=True,
            timeout=timeout,        # avoid hanging forever
        )
...
def lsgpu(show_json=False):

    if show_json:
        with Pool(len(clusters)) as p:
            results = p.imap_unordered(
                partial(showGPUs_fn, show_json=show_json),
                clusters
            )
            for cluster, status in results:
                cluster_status[cluster] = json.loads(status)
        print(json.dumps(cluster_status))
...
def print_vram_usage(mem_thresh, show_spec=False, show_json=False):
    info = get_gpu_uuid_to_info()
        ...
        if '-' in part:
            ...
            if m1 and m2 and m1.group(1) == m2.group(1):
                ...
                for i in range(s, e+1):
                    node = f"{prefix}{i}"
                    for j in range(8):
                        whitelist[(node, j)] = True

                continue
        # fallback: just append whatever it was
        if "g" in part:
            node = part.split("g")[0]
            gpu = int(part.split("g")[-1])
            whitelist[(node, gpu)] = True
        else:
            node = part
            for i in range(8):
                whitelist[(node, i)] = True
...
def find_free_nodes(clusters, whitelist, get_first=True):
    free_nodes = []
    with Pool(len(clusters)) as p:
        results = p.imap_unordered(
            partial(showGPUs_fn, show_json=True, timeout=10),
            clusters
        )
        for cluster, status in results:
            try:
            ...
            for gpu in cluster_status[cluster]:
                if whitelist and (cluster, gpu["idx"]) not in whitelist:
                    continue
                pct = gpu.get("pct", 100)
                if pct < args.threshold:
                    free_nodes.append((cluster, gpu["idx"]))
                    if get_first:
                        print(f"{cluster} {gpu['idx']}")
                        return


    if free_nodes:
        print("Free nodes (all GPUs <10% used):")
        for n in free_nodes:
            print(f" - {n}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--find', action='store_true', help='find free gpus')
    args = parser.parse_args()

    if args.nodes:
        clusters, whitelist = expand_nodes(args.nodes)
    else:
        whitelist = None
    ...
    if args.local:
        if args.json or args.compact and not args.full:
        ...
        find_free_nodes(clusters, whitelist, get_first=True)
    else:
        lsgpu(args.json)
